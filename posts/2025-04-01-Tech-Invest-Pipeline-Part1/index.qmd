...
g2<-ggplot(py$df,aes(x=as_date(date)))+
    geom_hline(yintercept=0,linewidth=.5,color="#000000")+
    geom_col(aes(y=histogram,fill=MACD>signal),color="#000000")+
    scale_x_date(
...
g1<-ggplot(py$df,aes(x=as_date(date)))+
    ...
    geom_line(aes(y=signal),color=r,linewidth=1.1)+
    geom_line(aes(y=MACD),color=bg,linewidth=1.1)+
    scale_x_date(
        limits=c(l,u),
        date_breaks="1 week",
        date_minor_breaks="1 day",
...
g3<-ggplot(py$df,aes(x=as_date(date)))+
    theme_update(
        axis.text.x=element_text(angle=45,hjust=1,vjust=1,color="#ffffff"),
        axis.ticks.x=element_line(),
    )+
    annotate(geom="rect",fill="#000000",xmin=l,xmax=u,ymin=30,ymax=70,alpha=0.4)+
    geom_line(aes(y=RSI),linewidth=1.1,color=bg)+
    scale_x_date(
        limits=c(l,u),
        date_breaks="1 week",
        date_minor_breaks="1 day",
...
p<-ggplot(py$df,aes(x=as_date(date)))+
    geom_line(aes(y=sma_l),color=r,linewidth=1.1)+
    geom_line(aes(y=sma_s),color=bg,linewidth=1.1)+
    geom_linerange(aes(ymax=high,ymin=low),color=r)+
    geom_tile(
        aes(
            y=candle_mid,
            height=(candle_high-candle_low)+.14,
            width=.94,
        ),
        fill="#000000", 
    )+
...
More to come (including some color scheme fussing), but that's it for now!