...
1.  ingest a largeish csv file (\~25GB) with `r prettyNum(params$rows,big.mark=",")` records and `r prettyNum(params$cols,big.mark=",")` columns of random normal variates plus one `ID` column,
...
There will obviously be differences from one system to the next (and there certainly are between my Mac at home and my Windows laptop at work), but in general we can see there's a sizable penalty for the plain vanilla `dplyr` approach. And if you've done much heavy data work with it, you've probably experienced it yourself. At least in this example, `Python` with `polars` is nearly identical to `dtplyr` and `tidytable` is just a tad slower.